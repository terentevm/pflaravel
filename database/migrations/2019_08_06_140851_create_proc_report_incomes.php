...
class CreateProcReportIncomes extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement('CREATE OR REPLACE FUNCTION public.report_incomes(IN user_id uuid,IN date_begin date,IN date_end date,IN currency_id_to uuid,IN period_mode integer DEFAULT 0)
    ...
    COST 100    ROWS 1000 
AS $BODY$BEGIN
  DROP TABLE IF EXISTS tmp_incomes;
  ...
  RETURN QUERY SELECT
               ...
               FROM tmp_incomes as t1
                      LEFT JOIN ref_items_income as items on t1.item = items.id
               GROUP BY
                        CASE
                          WHEN period_mode = 0 THEN NOW()
                          WHEN period_mode = 1 THEN t1.period::date
...
$BODY$;');
    ...
    {
        DB::statement('DROP FUNCTION report_incomes(user_id uuid,date_begin date,date_end date,currency_id_to uuid, period_mode integer)');
    }
}
